					...
					<div class="col-6">
						<div class="invisible" data-toggle="appear" data-class="animated fadeInRight">
							<div class="font-size-sm font-w600 text-uppercase text-success">
								<?php
								$query = $this->db->query('SELECT seleksi_berkas FROM manlan_admin where sebagai = "calonasisten" and seleksi_berkas ="ya"');
								echo "Lulus : ", $query->num_rows();
								?>
							</div>
							<div class="font-size-sm font-w600 text-uppercase text-warning">
								<?php
								$query = $this->db->query('SELECT seleksi_berkas FROM manlan_admin where sebagai = "calonasisten" and seleksi_berkas =" "');
								echo "Proses : ", $query->num_rows();
								?>
							</div>
							<div class="font-size-sm font-w600 text-uppercase text-danger">
								<?php
								$query = $this->db->query('SELECT seleksi_berkas FROM manlan_admin where sebagai = "calonasisten" and seleksi_berkas ="tidak"');
				...
				<div class="row py-20">
					...
					<div class="col-6">
						<div class="invisible" data-toggle="appear" data-class="animated fadeInRight">
							<div class="font-size-sm font-w600 text-uppercase text-success">
								<?php
								$query = $this->db->query('SELECT seleksi_berkas FROM manlan_admin where sebagai = "calonprogrammer" and seleksi_berkas ="ya"');
								echo "Lulus : ", $query->num_rows();
								?>
							</div>
							<div class="font-size-sm font-w600 text-uppercase text-warning">
								<?php
								$query = $this->db->query('SELECT seleksi_berkas FROM manlan_admin where sebagai = "calonprogrammer" and seleksi_berkas =" "');
								echo "Proses : ", $query->num_rows();
								?>
							</div>
							<div class="font-size-sm font-w600 text-uppercase text-danger">
								<?php
								$query = $this->db->query('SELECT seleksi_berkas FROM manlan_admin where sebagai = "calonprogrammer" and seleksi_berkas ="tidak"');
								echo "Tidak : ", $query->num_rows();
								?>
							</div>
						</div>
